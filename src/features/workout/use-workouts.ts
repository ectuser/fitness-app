...
export function useWorkouts() {
  ...
  const createWorkoutMutation = useMutation(workoutMutations.create(queryClient));
  const updateWorkoutMutation = useMutation(workoutMutations.update(queryClient));
  const deleteWorkoutMutation = useMutation(workoutMutations.delete(queryClient));
  const duplicateWorkoutMutation = useMutation(workoutMutations.duplicate(queryClient));
  const toggleCompleteMutation = useMutation(workoutMutations.toggleComplete(queryClient));
  const workouts = data ?? [];
  ...
  return {
    ...
    deleteWorkout: (id: string) =>
      deleteWorkoutMutation.mutateAsync({ id }),
    duplicateWorkout: (id: string, options?: DuplicateWorkoutInput['options']) =>
