import type { ExerciseStats, Workout, WorkoutExercise, WorkoutHistory, WeightUnit } from '@/types';
...
function collectExerciseSets(exerciseId: string, workouts: Workout[]): ExerciseSetSnapshot[] {
...
export function calculateExerciseStats(exerciseId: string, workouts: Workout[]): ExerciseStats | null {
...
export function buildExerciseHistory(exerciseId: string, workouts: Workout[]): WorkoutHistory[] {
...
export function findLastWorkoutExercise(exerciseId: string, workouts: Workout[]): WorkoutExercise | null {
  for (const workout of sortWorkoutsByDateDesc(getCompletedWorkouts(workouts))) {
    const workoutExercise = workout.exercises.find((entry) => entry.exerciseId === exerciseId);
