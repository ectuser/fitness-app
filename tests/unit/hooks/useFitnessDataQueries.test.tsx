...
describe('fitness data query hooks', () => {
  ...
  it('reads exercises, workouts, and settings from localStorage-backed queries', () => {
    ...
    localStorage.setItem(
      ...
      JSON.stringify([completedBenchWorkout]),
    )
    localStorage.setItem(
    ...
    const { result: exercisesResult } = renderHook(() => useExercises(), {
      wrapper,
    })
    ...
    expect(queryClient.getQueryData(fitnessQueryKeys.exercises())).toEqual(
      exercises,
    )
    expect(queryClient.getQueryData(fitnessQueryKeys.workouts())).toEqual([
      completedBenchWorkout,
    ])
    expect(queryClient.getQueryData(fitnessQueryKeys.settings())).toEqual({
  ...
  it('persists migrated exercise data when reading from storage', () => {
    ...
    expect(
      JSON.parse(localStorage.getItem(STORAGE_KEYS.EXERCISES) ?? '[]')[0]
        .muscleGroups,
    ).toEqual(['Arms (Legacy)'])
  })
  ...
  it('persists exercise mutations through query data instead of component state', async () => {
    ...
    await waitFor(() => {
      expect(
        result.current.getExerciseById('77777777-7777-7777-7777-777777777777')
          ?.name,
      ).toBe('Weighted Pull-up')
    })
  })
  ...
  it('persists workout mutations and exposes derived workout lists', async () => {
    ...
    await waitFor(() => {
      expect(result.current.completedWorkouts[0].id).toBe(
        '88888888-8888-8888-8888-888888888888',
      )
    })
