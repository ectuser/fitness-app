...
describe('workout editor helpers', () => {
  ...
  it('adds, replaces, moves, removes, and updates workout exercises', () => {
    ...
    const appended = addOrReplaceWorkoutExercise(
      initialExercises,
      exercises[2],
      [],
      'kg',
    ...
    expect(
      moveWorkoutExercise(initialExercises, 1, 'up').map(
        (exercise) => exercise.exerciseId,
      ),
    ).toEqual(['exercise-row', 'exercise-bench'])
  ...
  it('returns the replacement filter group and validates workout forms', () => {
    ...
    expect(
      getExerciseReplacementFilterGroup(workoutExercises, exercises, 1),
    ).toBe('Back')
  ...
  it('formats default names and leaves out-of-bounds moves unchanged', () => {
    expect(formatDefaultWorkoutName(new Date('2026-04-01T00:00:00.000Z'))).toBe(
      'Wed, April 1',
    )
    ...
    expect(moveWorkoutExercise(workoutExercises, 0, 'up')).toEqual(
      workoutExercises,
    )
    expect(moveWorkoutExercise(workoutExercises, 1, 'down')).toEqual(
      workoutExercises,
    )
