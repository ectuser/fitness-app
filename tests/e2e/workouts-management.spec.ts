...
test('workout edit and save changes', async ({ page }) => {
    ...
    workouts: [
      buildWorkout({
        ...
        exercises: [
          ...
          buildWorkoutExercise('exercise-pullups', 1, [
            buildSet('set-pull-1', 0, 10),
          ]),
        ],
      }),
    ],
  ...
  await expect(page.getByRole('heading', { name: 'Workouts' })).toBeVisible()
  await expect(
    page.getByRole('heading', { name: 'Upper Mix Updated' }),
  ).toBeVisible()
  await expect(page.getByText('2 exercises')).toBeVisible()
})

test('workout menu duplicate delete and complete/incomplete transitions', async ({
  page,
}) => {
  await seedAppStorage(page, {
    exercises: [
  ...
  await expect(
    page.getByRole('heading', { name: 'Menu Day (Copy)' }),
  ).toBeVisible()

  await workoutCard('Menu Day (Copy)').getByRole('button').first().click()
  await page.getByRole('menuitem', { name: 'Delete' }).click()
  await expect(page.getByRole('button', { name: 'Cancel' })).not.toHaveClass(
    /border-input/,
  )
