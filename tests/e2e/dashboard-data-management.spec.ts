...
test('dashboard export downloads the current app data', async ({ page }) => {
  ...
  expect(download.suggestedFilename()).toMatch(
    /^fitness-app-backup-\d{4}-\d{2}-\d{2}\.json$/,
  )
  ...
  expect(exportPayload.data.workouts).toEqual(
    expect.arrayContaining([
      expect.objectContaining({ name: 'Export Workout' }),
    ]),
  )
  ...
  await page.evaluate(
    ({ keys, exercises, workouts }) => {
      ...
      localStorage.setItem(
        keys.SETTINGS,
        JSON.stringify({ defaultWeightUnit: 'kg' }),
      )
    },
    {
      keys: STORAGE_KEYS,
    ...
    .poll(async () => {
      return page.evaluate(() => {
        ...
        const settings = JSON.parse(
          localStorage.getItem('fitness-app-settings') || '{}',
        )
