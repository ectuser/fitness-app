...
test('app restores last visited page and falls back safely for invalid saved route', async ({
  page,
}) => {
  ...
  await expect
    .poll(async () => {
      return page.evaluate(() => {
        return JSON.parse(
          localStorage.getItem('fitness-app-last-visited-path') || '""',
        )
      })
    })
    .toBe('/exercises')
  ...
  await expect(
    reopenedPage.getByRole('heading', { name: 'Exercises' }),
  ).toBeVisible()
  await expect(reopenedPage).toHaveURL(/\/exercises$/)

  await reopenedPage.evaluate(() => {
    localStorage.setItem(
      'fitness-app-last-visited-path',
      JSON.stringify('/not-a-real-route'),
    )
  })
  ...
  await expect(
    invalidRoutePage.getByRole('heading', { name: 'Dashboard' }),
  ).toBeVisible()
