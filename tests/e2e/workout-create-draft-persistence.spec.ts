...
const draftValue = (page: Page) =>
  page.evaluate((draftKey) => localStorage.getItem(draftKey), STORAGE_KEYS.WORKOUT_CREATE_DRAFT);
...
test('create workout restores full draft after navigating away and returning', async ({ page }) => {
  ...
  await expect(page.getByLabel('Workout Name')).toHaveValue('Draft Restore Workout');
  ...
  await expect(restoredBenchCard.getByPlaceholder('Weight').first()).toHaveValue('82.5');
  await expect(restoredBenchCard.getByPlaceholder('Reps').first()).toHaveValue('7');
  await restoredBenchCard.getByRole('button', { name: 'Details' }).click();
  await expect(restoredBenchCard.getByLabel('Comment')).toHaveValue('Top set felt strong');
});

test('create workout clears draft key after successful create', async ({ page }) => {
  await seedExercises(page);
  await page.goto('/workouts/new');
...
test('save and finish workout clears draft key after successful save', async ({ page }) => {
  await seedExercises(page);
  await page.goto('/workouts/new');
