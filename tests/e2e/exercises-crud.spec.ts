...
test('exercise create and edit custom flow', async ({ page }) => {
  ...
  await expect(
    page.getByRole('heading', { level: 1, name: 'Cable Row' }),
  ).toBeVisible()
...
test('exercise delete unused custom succeeds', async ({ page }) => {
  ...
  await expect(
    page.getByRole('heading', { level: 1, name: 'Custom Pulldown' }),
  ).toBeVisible()

  await page.getByRole('button').nth(2).click()
  await expect(
    page.getByRole('heading', { name: 'Delete Exercise?' }),
  ).toBeVisible()
...
test('exercise delete used in workouts shows guard error', async ({ page }) => {
  ...
  await expect(
    page.getByRole('heading', { level: 1, name: 'Protected Curl' }),
  ).toBeVisible()
  ...
  await expect
    .poll(() => dialogMessage)
    .toContain('Cannot delete exercise that is used in workouts')
