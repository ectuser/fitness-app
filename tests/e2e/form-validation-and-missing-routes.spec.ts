...
test('exercise form validates required fields before saving', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Save Exercise' }).click();
  await expect(page.getByText('Please select at least one muscle group')).toBeVisible();
  await expect(page).toHaveURL(/\/exercises\/new$/);

  await expect.poll(async () => {
    return page.evaluate(() => {
      const exercises = JSON.parse(localStorage.getItem('fitness-app-exercises') || '[]');
      return exercises.some((exercise: { name: string }) => exercise.name === 'Validation Curl');
    });
  }).toBe(false);
});

test('workout form validates required fields before saving', async ({ page }) => {
  await clearAppStorage(page);
...
test('missing exercise detail route falls back to the exercises list', async ({ page }) => {
  await clearAppStorage(page);
...
test('missing workout session route falls back to the workouts list', async ({ page }) => {
  await clearAppStorage(page);
  await page.goto('/workouts/not-a-real-workout/session');

  await expect(page.getByRole('heading', { name: 'Workout Not Found' })).toBeVisible();
