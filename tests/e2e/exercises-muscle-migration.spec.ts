...
import { buildExercise, seedAppStorage } from './helpers/storage'

test('muscle taxonomy migration and exercise form options', async ({
  page,
}) => {
    ...
    .poll(async () => {
      return page.evaluate(() => {
        ...
        const byName = Object.fromEntries(
          exercises.map(
            (exercise: { name: string; muscleGroups: Array<string> }) => [
              exercise.name,
  ...
  await expect(page.getByRole('button', { name: 'Arms (Legacy)' })).toHaveCount(
    0,
  )
  await expect(page.getByRole('button', { name: 'Legs (Legacy)' })).toHaveCount(
    0,
  )
})
