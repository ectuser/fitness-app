...
test('workout session add exercise finish and update stats', async ({
  page,
}) => {
    ...
    .poll(async () => {
      return page.evaluate(() => {
        ...
        const workout = workouts.find(
          (w: { id: string }) => w.id === 'workout-session-1',
        )
  ...
  await expect(
    page.getByRole('heading', { level: 3, name: 'Push-ups' }),
  ).toBeVisible()
  ...
  await expect(page.getByText('6.52 kg', { exact: true })).toBeVisible()
