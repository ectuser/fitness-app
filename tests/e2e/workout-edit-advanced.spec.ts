...
test('workout edit supports reordering exercises, deleting exercises, and persisting set changes', async ({
    ...
    workouts: [
      buildWorkout({
        ...
        exercises: [
          ...
          buildWorkoutExercise('edit-advanced-pullups', 1, [
            buildSet('pullups-set-1', 0, 10),
          ]),
          buildWorkoutExercise('edit-advanced-squats', 2, [
            buildSet('squats-set-1', 90, 6),
          ]),
        ],
      }),
    ],
  ...
  await workoutExerciseCard(page, 'Squats')
    .getByRole('button', { name: 'Add Set' })
    .click()
  await workoutExerciseCard(page, 'Squats')
    .getByPlaceholder('Weight')
    .nth(1)
    .fill('100')
  await workoutExerciseCard(page, 'Squats')
    .getByPlaceholder('Reps')
    .nth(1)
    .fill('5')
    ...
    .poll(async () => {
      return page.evaluate(() => {
        ...
        const workout = workouts.find(
          (entry: { id: string }) => entry.id === 'workout-edit-advanced',
        )
...
test('workout edit can switch an exercise and keep last completed workout defaults', async ({
  ...
  await workoutExerciseCard(page, 'Push-ups')
    .getByRole('button', { name: 'Details' })
    .click()
  await expect(
    workoutExerciseCard(page, 'Push-ups').getByLabel('Comment'),
  ).toHaveValue('Pause at the bottom')
  await expect(
    workoutExerciseCard(page, 'Push-ups').getByPlaceholder('Weight').first(),
  ).toHaveValue('20')
  await expect(
    workoutExerciseCard(page, 'Push-ups').getByPlaceholder('Reps').first(),
  ).toHaveValue('15')
  ...
  await expect
    .poll(async () => {
      return page.evaluate(() => {
        ...
        const workout = workouts.find(
          (entry: { id: string }) => entry.id === 'switch-edit-workout',
        )
    ...
    .toEqual(
      expect.objectContaining({
        ...
        sets: [
          { id: expect.any(String), weight: 20, weightUnit: 'kg', reps: 15 },
        ],
      }),
    )
})
...
test('workout edit menu returns to workout form after exercise save and preserves draft state', async ({
  ...
  await workoutExerciseCard(page, 'Editable Fly')
    .getByPlaceholder('Weight')
    .first()
    .fill('15')
  await workoutExerciseCard(page, 'Editable Fly')
  ...
  await expect(
    page.getByRole('heading', { name: 'Editable Fly Updated' }),
  ).toBeVisible()
  await expect(
    workoutExerciseCard(page, 'Editable Fly Updated')
      .getByPlaceholder('Weight')
      .first(),
  ).toHaveValue('15')
  await expect(
...
test('workout create returns to form after exercise save and keeps unsaved draft state', async ({
  ...
  await workoutExerciseCard(page, 'Draft Fly')
    .getByPlaceholder('Weight')
    .first()
    .fill('11')
  await workoutExerciseCard(page, 'Draft Fly')
  ...
  await expect(
    workoutExerciseCard(page, 'Draft Fly Updated')
      .getByPlaceholder('Weight')
      .first(),
  ).toHaveValue('11')
  await expect(
...
test('workout edit menu returns to workout form after exercise cancel and preserves draft state', async ({
  ...
  await workoutExerciseCard(page, 'Editable Raise')
    .getByPlaceholder('Weight')
    .first()
    .fill('14')
  await workoutExerciseCard(page, 'Editable Raise')
  ...
  await expect(page.getByLabel('Workout Name')).toHaveValue(
    'Cancel Entry Workout Draft',
  )
  await expect(
    page.getByRole('heading', { name: 'Editable Raise' }),
  ).toBeVisible()
  await expect(
    page.getByRole('heading', { name: 'Editable Raise Should Not Save' }),
  ).toHaveCount(0)
  await expect(
    workoutExerciseCard(page, 'Editable Raise')
      .getByPlaceholder('Weight')
      .first(),
  ).toHaveValue('14')
  await expect(
...
test('workout create returns to form after exercise cancel and preserves draft state', async ({
  ...
  await workoutExerciseCard(page, 'Draft Press')
    .getByPlaceholder('Weight')
    .first()
    .fill('22')
  await workoutExerciseCard(page, 'Draft Press')
  ...
  await expect(workoutExerciseCard(page, 'Draft Press')).toBeVisible()
  await expect(
    page.getByRole('heading', { name: 'Draft Press Should Not Save' }),
  ).toHaveCount(0)
  await expect(
    workoutExerciseCard(page, 'Draft Press').getByPlaceholder('Weight').first(),
  ).toHaveValue('22')
  await expect(
    workoutExerciseCard(page, 'Draft Press').getByPlaceholder('Reps').first(),
  ).toHaveValue('6')
})
