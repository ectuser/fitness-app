...
test('exercise selector can create a new exercise inline and add it to the workout', async ({
  ...
  const createExerciseDialog = page.getByRole('dialog', { name: 'Create New Exercise' });
  await expect(createExerciseDialog).toBeVisible();
  await createExerciseDialog.getByPlaceholder('e.g., Barbell Rows').fill('Zottman Curl');
  ...
  await createExerciseDialog.getByRole('button', { name: 'Save Exercise' }).click();

  await expect(page.getByRole('heading', { level: 3, name: 'Zottman Curl' })).toBeVisible();
  ...
  await expect(page.getByRole('heading', { name: 'Inline Exercise Workout' })).toBeVisible();
  ...
  await expect.poll(async () => {
    return page.evaluate(() => {
      ...
      const exercise = exercises.find((entry: { name: string }) => entry.name === 'Zottman Curl');
      const workout = workouts.find((entry: { name: string }) => entry.name === 'Inline Exercise Workout');
